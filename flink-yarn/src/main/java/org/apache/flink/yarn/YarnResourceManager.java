...
public class YarnResourceManager extends ResourceManager<ResourceID> {
	...
	/**
	 * Environment variable name of the final container id used by the YarnResourceManager. Container ID generation may vary across Hadoop versions.
	 */
	final static String ENV_FLINK_CONTAINER_ID = "_FLINK_CONTAINER_ID";

	/**
	 * Environment variable name of the hostname given by the YARN. In task executor we use the hostnames given by YARN consistently throughout akka
	 */
	...
	public YarnResourceManager(Configuration flinkConfig, Map<String, String> env, RpcService rpcService,
			ResourceManagerConfiguration resourceManagerConfiguration, HighAvailabilityServices highAvailabilityServices, SlotManagerFactory slotManagerFactory,
			MetricRegistry metricRegistry, JobLeaderIdService jobLeaderIdService, FatalErrorHandler fatalErrorHandler) {
		super(rpcService, resourceManagerConfiguration, highAvailabilityServices, slotManagerFactory, metricRegistry, jobLeaderIdService, fatalErrorHandler);
		this.flinkConfig = flinkConfig;
		this.yarnConfig = new YarnConfiguration();
		this.ENV = env;
		final int yarnHeartbeatIntervalMS = flinkConfig.getInteger(ConfigConstants.YARN_HEARTBEAT_DELAY_SECONDS, DEFAULT_YARN_HEARTBEAT_INTERVAL_MS / 1000)
				* 1000;
	...
	protected void initialize() throws ResourceManagerException {
		resourceManagerClient = AMRMClientAsync.createAMRMClientAsync(yarnHeartbeatIntervalMillis, new YRMCallbackHandler());
	...
	private ContainerLaunchContext createTaskExecutorLaunchContext(Resource resource, String containerId, String host) throws Exception {
		...
		final ContaineredTaskManagerParameters taskManagerParameters = ContaineredTaskManagerParameters.create(flinkConfig, resource.getMemorySize(), 1);

		LOG.info("TaskExecutor{} will be started with container size {} MB, JVM heap size {} MB, " + "JVM direct memory limit {} MB", containerId,
		...
		ContainerLaunchContext taskExecutorLaunchContext = Utils.createTaskExecutorContext(flinkConfig, yarnConfig, ENV, taskManagerParameters,
				taskManagerConfig, currDir, YarnTaskExecutorRunner.class, LOG);
	...
	/**
	 * Generate priority by given resource profile. Priority is only used for distinguishing request of different resource.
	 * 
	 * @param resourceProfile
	 *            The resource profile of a request
	...
	private class YRMCallbackHandler extends AMRMClientAsync.AbstractCallbackHandler {
		...
		public void onContainersAllocated(List<Container> containers) {
			for (Container container : containers) {
				...
				try {
					...
					ContainerLaunchContext taskExecutorLaunchContext = createTaskExecutorLaunchContext(container.getResource(), container.getId().toString(),
